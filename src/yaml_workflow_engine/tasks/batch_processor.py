...
from typing import Any, Dict, List, Optional, Tuple, Set, Iterator, Callable, Union
...
class BatchProcessor:
        ...
        if self.state_file.exists():
            try:
                with open(self.state_file) as f:
                    ...
                    self.logger.info(f"Loaded state: {len(processed)} processed, {len(failed)} failed")
    ...
    def process_item(
        ...
        try:
            ...
            if 'output' in task_config:
                ...
                step['output'] = str(self.workspace / template.render(
                    item=item,
                    batch_index=task_config.get('batch_index', 0),
                    batch=task_config.get('batch', []),
                    **context
            ...
            if 'file_output' in task_config:
                ...
                step['file_output'] = str(self.workspace / template.render(
                    item=item,
                    batch_index=task_config.get('batch_index', 0),
                    batch=task_config.get('batch', []),
                    **context
    ...
    def process_batch(
        ...
        if resume_state:
            ...
            self.logger.info(f"Loaded state: {len(processed_items)} processed, {len(failed_items)} failed")
