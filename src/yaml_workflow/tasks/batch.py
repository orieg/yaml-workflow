...
from . import register_task, TaskConfig, get_task_handler
from ..exceptions import TaskExecutionError


def process_item(item: Any, task_config: Dict[str, Any], context: Dict[str, Any], workspace: str, arg_name: str, chunk_index: int = 0, item_index: int = 0, total: int = 0, chunk_size: int = 0) -> Any:
    """
...
def batch_task(config: TaskConfig) -> Dict[str, Any]:
    ...
    max_workers = int(processed.get("max_workers", min(chunk_size, os.cpu_count() or 1)))
    if max_workers <= 0:
    ...
    state["stats"]["success_rate"] = (state["stats"]["processed"] / state["stats"]["total"]) * 100.0
