...
def test_multiply_invalid_input(context, workspace):
    step = {"name": "multiply_invalid", "inputs": {"operation": "multiply", "numbers": []}}
...
def test_divide_numbers(context, workspace):
    step = {"name": "divide", "inputs": {"operation": "divide", "dividend": 10, "divisor": 2}}
    result = python_task(step, context, workspace)
    assert result["result"] == 5.0


def test_divide_by_zero(context, workspace):
    step = {"name": "divide_zero", "inputs": {"operation": "divide", "dividend": 10, "divisor": 0}}
...
def test_custom_handler(context, workspace):
    ...
    step = {
        "name": "custom",
        "inputs": {"operation": "custom", "handler": custom_func, "args": [5], "kwargs": {"y": 3}},
    }
    result = python_task(step, context, workspace)
