...
def test_resolve_inputs_type_preservation(engine):
    ...
    assert result["template_number"] == "42"  # Note: becomes string due to template rendering
    assert result["template_flag"] == "True"  # Note: becomes string due to template rendering
    assert result["template_list"] == "[1, 2, 3]"  # Note: becomes string due to template rendering
