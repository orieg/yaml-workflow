...
from yaml_workflow_engine.tasks.batch_processor import BatchProcessor, process_batch
...
def test_batch_with_error_handling(temp_workspace):
    ...
    result = process_batch({
        ...
        "processing_task": {
            ...
            "inputs": {
                "operation": "custom",
                "handler": lambda x: x if "error" not in x else ValueError(f"Error processing {x}")
            }
        }
    }, {}, temp_workspace)
